...
function CanvasEditor(elem, width, height) {
...
CanvasEditor.prototype.setRectlinktext = function (rectlinktext) {
	this.rectlinktext = rectlinktext;
}

CanvasEditor.prototype.setMode = function (mode) {
...
CanvasEditor.prototype.on = function (event, callback) {
...
CanvasEditor.prototype.returnRotatedPoint = function (x, y, cx, cy, a) {
...
CanvasEditor.prototype.fire = function (event) {
    if(this.listeners[event]) {
...
CanvasEditor.prototype.un = function (event, callback) {
...
CanvasEditor.prototype.in_array = function (v, a) {
    for(var i = a.length; i-- && a[i] != v;);
    return i
}

//from vX JS, is it at all strange that I'm using my own work?
CanvasEditor.prototype.array_remove = function (e, o) {
    var x = this.in_array(e, o);
    x != -1 ? o.splice(x, 1) : 0
}


CanvasEditor.prototype.is_selected = function (shape) {
    return this.in_array(shape, this.selected) != -1;
}

CanvasEditor.prototype.set_attr = function () {
    for(var i = 0; i < this.selected.length; i++) {
        this.selected[i].attr.apply(this.selected[i], arguments)
    }
}

CanvasEditor.prototype.set = function (name, value) {
    this.prop[name] = value;
    this.set_attr(name, value);
}

CanvasEditor.prototype.onMouseDown = function (x, y, target) {
    this.fire("mousedown")
    ...
    if(this.mode == "select" && !this.selectbox) {
        ...
        } else if(this.mode == "rectlink") {
		
	    //shape = this.draw.text(x, y, this.prop['text']).attr('font-size', 0)
            //shape.text = this.prop['text'];

	    //shape = this.draw.text(x, y, this.prop['recttext']).attr('font-size', 0);
            //shape.recttext = this.prop['recttext'];
	    //var textBBox = shape.getBBox();
	    //var textUnderline = this.draw.path("M"+textBBox.x+" "+(textBBox.y+textBBox.height)+"L"+(textBBox.x+textBBox.width)+" "+(textBBox.y+textBBox.height), x, y);
	    //this.addShape(textUnderline);
        ...
        if(shape) {
            ...
            if(this.prop.fontfamily) {
                shape.attr({
                    "font-family": this.prop.fontfamily
                })
            }
            this.addShape(shape)
        }
...
CanvasEditor.prototype.onMouseMove = function (x, y, target) {
    ...
    if(this.mode == "select" || this.mode == "delete") {
        if(this.selectbox) {
            ...
            if(this.action == "move") {
                ...
                } else if(this.selected[0].type == "rectlink") {
                    this.resize(this.selected[0], x - this.onGrabXY[0], y - this.onGrabXY[1], this.onGrabXY[0], this.onGrabXY[1])
...
CanvasEditor.prototype.getMarkup = function () {
    return this.draw.canvas.parentNode.innerHTML;
}


CanvasEditor.prototype.onDblClick = function (x, y, target) {
    this.fire("dblclick")
...
CanvasEditor.prototype.onMouseUp = function (x, y, target) {
    this.fire("mouseup")
    this.onGrabXY = null;
    ...
    } else if(this.selected.length == 0 && this.mode == "rectlink") {
    	var me = this;    	
	...
	iframe.id = "weblink" + weblinkindex;
	...
	document.body.appendChild(iframe);
        ...
        shape = this.draw.rect(x, y, 210, 160);
	shape.id = this.generateUUID();
	shape.attr({
		"fill": this.prop.fill,
		"stroke": this.prop.stroke,
        ...
        var btn = document.getElementById("weblink" + weblinkindex);
        btn.style.left = (x+10) + 'px';
        btn.style.top = (y+10) + 'px';    	
	
	document.getElementById("weblink" + weblinkindex).style.display = "block";
	weblinkindex++;
	/*var lbl = this.draw.text(x, y, this.prop['text']).attr( {
		font : '14px Helvetica, Arial',
		stroke : "none",
		opacity: 1,
		fill : "#fff"
	}),
	blanket = this.draw.rect().attr(lbl.getBBox()).attr({
		fill : this.prop['fill'],
		stroke : this.prop['stroke'],
		opacity: 0.5,
		cursor: "pointer"
	}).click(function () { alert(me.rectlinktext); });
	
	var st = this.draw.set(blanket,lbl);*/
	
	//this.addShape(st)
	//this.addShape(blanket)
